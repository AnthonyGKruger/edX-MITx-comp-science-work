...
def isValidWord(word, hand, wordList):
    ...
    for key in hand.keys():
        if key in word:
            check += ''.join(key)
            if hand[key] > 1 and word.count(key) > 1:
                 check += key
    sorted_check = ''.join(sorted(check))
    if sorted_word in sorted_check and word in wordList:
        valid = True
    else:
        valid = False
    return valid
