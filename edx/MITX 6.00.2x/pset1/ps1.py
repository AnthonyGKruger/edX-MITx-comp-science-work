...
def greedy_cow_transport(cows,limit=10):
    ...
    for k, v in cows.items():
        temp_cows[k] = v
    sorted_cows = sorted(temp_cows.items(), key=lambda x: x[1], reverse = True)

    while sum(temp_cows.values()) != 0:
        temp_list = []
        counter = 0
        for cows, weights in sorted_cows:
            if temp_cows[cows] != 0 and weights + counter <= limit:
                temp_list.append(cows)
                counter += weights
                temp_cows[cows] = 0
...
def brute_force_cow_transport(cows,limit=10):
    ...
    """
    # TODO: Your code here
    possibilities_list = sorted(get_partitions(cows), key=len)
    possibilities = []
    for lst in possibilities_list:
        ship = []
        for sublist in lst:
            ship_weight = []
            for cow in sublist:
                ship_weight.append(cows[cow])
            ship.append(sum(ship_weight))
        if all(weight <= limit for weight in ship):
            possibilities.append(lst)
    new_possibilities = []
    for lst in possibilities:
        if lst not in new_possibilities:
            new_possibilities.append(lst)
    smallest = min(map(len, new_possibilities))
    for lst in new_possibilities:
        if len(lst) == smallest:
            return lst
