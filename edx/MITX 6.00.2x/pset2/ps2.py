...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        coo = (int(pos.x), int(pos.y))
        if coo not in self.cleaned_tile:
            self.cleaned_tile.append(coo)
    ...
    def isTileCleaned(self, m, n):
        ...
        self.n = n
        tile = (self.m, self.n)
        return tile in self.cleaned_tile
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        if self.room.isPositionInRoom(new_position):
            ...
            self.setRobotDirection(random.randint(0,365))
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        steps_taken = 0
        robot_list = [robot_type(room, speed) for total_robots in range(num_robots)]
        while room.getNumCleanedTiles()/room.getNumTiles() < min_coverage:
            steps_taken += 1
            for robot in robot_list:
                robot.updatePositionAndClean()
            if room.getNumCleanedTiles() / room.getNumTiles() >= min_coverage:
                final_results.append(steps_taken)
            else:
                continue
...
class RandomWalkRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        new_position = self.getRobotPosition().getNewPosition(random.randint(0, 365), self.speed)
        if self.room.isPositionInRoom(new_position):
            self.setRobotPosition(new_position)
            self.setRobotDirection(random.randint(0, 365))
            self.room.cleanTileAtPosition(new_position)
